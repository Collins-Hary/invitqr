...
export default function ScannerPage() {
  ...
  useEffect(() => {
    if (!event || showBackupCodeInput) return
        ...
        await qrScanner.start(
          ...
          (decodedText) => {
            ...
            try {
              ...
              if (qrToken) handleCheckIn({ qrToken })
            } catch (e) {
  ...
  const handleBackupCodeSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (backupCode.length >= 6) handleCheckIn({ backupCode })
  }
  ...
  return (
    <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-black text-white">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 text-center">
        {showBackupCodeInput ? (
          <form onSubmit={handleBackupCodeSubmit} className="mx-auto w-full max-w-sm">
            <input
              type="text"
              value={backupCode}
          ...
          <button onClick={() => setShowBackupCodeInput(true)} className="inline-flex items-center gap-2 rounded-full bg-slate-800/80 px-4 py-2 text-sm font-medium text-white backdrop-blur-sm transition hover:bg-slate-700">
