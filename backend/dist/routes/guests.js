...
function drawInvitePdf(doc, event, guest, qrPng) {
...
router.post('/:eventId/guests', authMiddleware, async (req, res, next) => {
    try {
        const user = req.user;
...
router.get('/:eventId/guests', authMiddleware, async (req, res, next) => {
    try {
        const user = req.user;
        const eventId = req.params.eventId;
        const event = await prisma.event.findFirst({ where: { id: eventId, user_id: user.id } });
...
router.get('/:eventId/guests/:guestId', authMiddleware, async (req, res, next) => {
    try {
        const user = req.user;
...
router.patch('/:eventId/guests/:guestId', authMiddleware, async (req, res, next) => {
    try {
        const user = req.user;
        ...
        if (typeof body.rsvp_status === 'string')
            data.rsvp_status = body.rsvp_status;
...
router.delete('/:eventId/guests/:guestId', authMiddleware, async (req, res, next) => {
    try {
        const user = req.user;
        const guest = await prisma.guest.findFirst({ where: { id: req.params.guestId, event: { user_id: user.id } } });
        if (!guest) {
...
// ─── Send Invite ──────────────────────────────────────────────────────────────
router.post('/:eventId/guests/:guestId/send-invite', authMiddleware, async (req, res, next) => {
    try {
        const user = req.user;
        ...
        if (guest.email) {
            const result = await sendInviteEmail({ name: guest.name, email: guest.email, phone: guest.phone, backup_code: guest.backup_code, qr_token: guest.qr_token }, { name: event.name, date: event.date, location: event.location }, baseUrl);
            if (result.success) {
...
router.post('/:eventId/send-all-invites', authMiddleware, async (req, res, next) => {
    try {
        const user = req.user;
        const eventId = req.params.eventId;
        const event = await prisma.event.findFirst({ where: { id: eventId, user_id: user.id } });
